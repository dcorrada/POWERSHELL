...
#
# COMMENT:  Script to check for windows updates splitting between important and recommended with Nagios + NRPE/NSClient++
#
...
Start-Transcript -path 'Updates.txt' -append
...
write-host "`n"
Write-Host "Total Updates: $($Total.Count)"  -foregroundcolor "green"
write-host "`n"
...
Write-Host "Hidden Updates: $($Hidden.Count)"  -foregroundcolor "gray"
write-host "`n"
...
if (($($Critical.Count) + $($Optional.Count)) -gt 0) {
	...
	write-host "`n"
	Write-Host “$($Hidden.Count) Hidden Updates Found” -foregroundcolor "gray"
	$Hidden | Sort-Object HotFixID | Format-Table -AutoSize
}
...
if ($($Hidden.Count) -gt 0) {
	Write-Host "OK - $($Hidden.Count) hidden updates."
	exit $returnStateOK
}
